# The code is adapted from VTDepth:
...
def load_weights(encoder):
    try:
        ckpt_path = "encoder_pretrained_weights\SwiftFormer_S_ckpt.pth" #weights pretrained on imagenet
...
def build_depth(pretrained=True):
    encoder = SwiftFormer_S()
    if pretrained:
        encoder = load_weights(encoder)
    decoder = networks.DepthDecoder(num_ch_enc=encoder.embed_dims, enc_name="SwiftFormer_S")
